...
SELECT 'TG7', encode(ST_AsMVT('test', 4096, 'geom', q
	...
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, false)
    AS geom) AS q;
SELECT 'TG8', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (SELECT 1 AS c1,
    ST_AsMVTGeom(ST_GeomFromText('POINT(25 17)'),
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, true)
    AS geom) AS q;
SELECT 'TG9', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (SELECT 1 AS c1,
    ST_AsMVTGeom(ST_GeomFromText('MULTIPOINT(25 17, -26 -18)'),
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, true)
    AS geom) AS q;
...
SELECT 'TA1', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (SELECT 1 AS c1, 'abcd'::text AS c2,
    ST_AsMVTGeom(ST_GeomFromText('POINT(25 17)'),
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, false)
    AS geom) AS q;
SELECT 'TA2', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (SELECT 1.1::double precision AS c1,
    ST_AsMVTGeom(ST_GeomFromText('POINT(25 17)'),
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, false)
    AS geom) AS q;
SELECT 'TA3', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (SELECT NULL::integer AS c1,
    ST_AsMVTGeom(ST_GeomFromText('POINT(25 17)'),
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, false)
    AS geom) AS q;
SELECT 'TA4', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (
	...
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, false)
    AS geom) AS q;
SELECT 'TA5', encode(ST_AsMVT('test', 4096, 'geom', q
), 'base64') FROM (SELECT ST_AsMVTGeom(ST_GeomFromText('POINT(25 17)'),
	ST_MakeBox2D(ST_Point(0, 0), ST_Point(4096, 4096)), 4096, 0, false)
    AS geom, 1 AS c1, 'abcd'::text AS c2) AS q;

-- unsupported input
SELECT 'TU1';
SELECT encode(ST_AsMVT('test', 4096, 'geom', NULL
), 'base64');
SELECT 'TU2';
SELECT encode(ST_AsMVT('test', 4096, 'geom', 1
), 'base64');
SELECT 'TU3';
SELECT encode(ST_AsMVT('test', 4096, 'geom', q
